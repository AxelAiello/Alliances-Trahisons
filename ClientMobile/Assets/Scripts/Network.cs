	...
	private bool DoOpen() {
		...
		if (socket == null) {
			...
			socket.On ("reconnected", (data) => {
				Debug.Log("Data reconnected : " + data);
				//JObject node = JObject.Parse(data.ToString());
				//Player.initializeCurrentPlayer((int) node["userId"], this.inputPseudo.text, "NO_COLOR");

				Debug.Log("Emit reconnected : " + JsonConvert.SerializeObject(Player.CurrentPlayer));
				socket.Emit ("reconnected", JsonConvert.SerializeObject(Player.CurrentPlayer));
			});
