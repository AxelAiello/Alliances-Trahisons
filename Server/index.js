...
io.on('connection', function(socket) {
  ...
  /***************************** VERSION FOR LOCAL - WITHOUT TABLE *********************************/
  ...
  if(userId <= 4) {
    ...
    // Conquer Planet
    console.log('Conquer a planet n°' + (userId * 3 - 3) + ' for Player n°' + (userId - 1));
    console.log('Conquer a planet n°' + (userId * 3 - 2) + ' for Player n°' + (userId - 1));
    console.log('Conquer a planet n°' + (userId * 3 - 1) + ' for Player n°' + (userId - 1));
    // Remove planet
    for(var i = 0; i < planets.length; i++) {
        if(planets[i].id == (userId * 3 - 1) || planets[i].id == (userId * 3 - 2) || planets[i].id == (userId * 3 - 3)) {
            planets[i].id_player = (userId - 1);
        }
    }

    // Add a Fleet
    console.log('Add a fleet to Player n°' + (userId - 1) + " on planet n°" + (userId - 1));
    // Add fleet
    fleets.push({
      id: fleetId,
      name: fleetId,
    ...
    for(var i = 0; i < players[(userId - 1)].resources.length; i++)
        players[(userId - 1)].resources[i]--;
    ...
    socket.emit('reconnected', {userId: socket.userId});
  }
  ...
  // "Connected" event
  socket.on('reconnected', function(message){
    message = JSON.parse(message);
    console.log(message.Pseudo + ' is reconnected');
    for(var i = 0; i < players.length; i++) {
        if(players[i].pseudo == message.Pseudo) {
          // Adress of user
          users[socket.userId] = {id : 0};
          users[players[i].id] = {id : socket.id};
          socket.userId = players[i].id;
          // "Connect" event
          console.log('a user reconnected, Player n°: ' + socket.userId);
        }
    }
    // Update Client
    update_client();
